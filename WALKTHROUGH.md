...
## Press and hold a sticker flat

In **Preview**, press and hold a visible part of a sticker to flatten its corner. Release anywhere to restore its curl; the usual nearby-cursor hover response can resume afterward. Arrange keeps image dragging and corner editing. A floating sticker flattens at its existing wall offset, rather than moving toward the wall. Transparent PNG holes do not catch presses, and the bulb or nearer content takes priority when it covers the sticker.

`src/sticker-press.ts` owns pointer capture, temporary influence, easing, and release/cancellation cleanup. `src/content.ts` combines press and hover influences on the existing vertex buffer. Full press sets displayed curl to zero, while the saved record stays untouched; shadows and refraction use the displayed geometry. `src/scene.ts` performs alpha-aware picking and keeps rendering during transitions, then sleeps. `src/editor.ts` explains the gesture in Preview. There is no project-format or public-API change.

Tune **`STICKER.press` in `src/config.ts`**: `flattenTime: 0.08` and `releaseTime: 0.18` are seconds and are independent exponential time constants. About 95% of each transition completes in three time constants (240 / 540 ms). For a quicker return, change `releaseTime` to `0.12` (about 360 ms to 95%). Keep both positive. These are appearance choices, not engine limits. `settleEpsilon: 0.0001` snaps the final influence to its exact target so the mesh becomes completely flat and idle rendering can stop; `maxDelta: 0.05` seconds bounds time catch-up. Saved curl and peel area still determine the restored shape; no linked project values need changing.

Reduced motion makes this direct interaction immediate. Cancellation, leaving Preview, blur, hiding the page, project loading, replacement, deletion, and disposal clear temporary press state. Release outside the sticker and lost pointer capture restore normally. The unit tests live in `tests/sticker-press.test.ts`; `tests/sticker-press-checks.ts` exercises the real scene and joins the browser suite.

Validation: **33 unit tests pass**, the production build passes, and Chrome reports **ALL CHECKS PASSED** with no captured console/runtime errors. The new checks cover transparent-hole picking, full flattening, unchanged JSON, reused buffers, shadow restoration, release outside the sticker, cancellation/capture loss, Arrange dragging, floating offsets, project loading and removal while pressed. Existing hover, filament, PNG, glass, bulb release, resize and disposal regressions also pass. The 390 × 700 Chrome comparison remains visually intact. Safari's diagnostics tab remained loading after selection and refresh, so Safari is unverified for this update. Real touch/pen hardware and a physical hold gesture remain unverified; browser gesture checks use synthetic pointer events. Vite retains the existing bundle-size advisory.
